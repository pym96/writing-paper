    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(image_batch.shape[1]):
                ...
                single_image = image_batch[:, i, :, :, :]  # 获取单张图像
                # 直接使用 pre_mask，因为它适用于所有图像
                single_pre_mask = pre_mask.clone()  # 克隆 pre_mask 以避免在原地修改
                single_pre_mask.retain_grad()
                
                print(f"\n single_image batch shape: {single_image.shape}")
                print(f"\n single_pre_mask mask shape: + {single_pre_mask.shape}")
                
                for i in range(args.iters):

                    foreground = single_image * single_pre_mask
                    background = single_image * (1-single_pre_mask)

                    pred_foreground = inpainter(background, (1-single_pre_mask))
                    pred_background = inpainter(foreground, single_pre_mask)

                    # # inpainting error is equiv to negative coeff. of constraint between foreground and background
                    inp_error = neg_coeff_constraint(single_pre_mask, pre_mask, pred_foreground, pred_background)
                    # # diversity term is the total deviation of foreground and background pixels
                    mask_diversity = diversity(single_image, single_pre_mask, foreground, background)
                    ...
                    with torch.no_grad():
                        grad = single_pre_mask.grad.data
                        # we only update mask pixels that are in the boundary AND have non-zero gradient
                        update_bool = boundary(single_pre_mask) * (grad != 0)
                        # pixels with positive gradients are set to 1 and with negative gradients are set to 0
                        single_pre_mask.data[update_bool] = (grad[update_bool] > 0).float()
                        grad.zero_()
                        
                        # smoothing procedure: we set a pixel to 1 if there are 4 or more 1-valued pixels in its 3x3 neighborhood
                        single_pre_mask.data = (F.avg_pool2d(single_pre_mask, 3, 1, 1, divisor_override=1) >= 4).float()
                        
                        updated_masks.append(single_pre_mask)
                             
            # TODO: Things to do end
            pre_mask = torch.stack(updated_masks, dim=1)
